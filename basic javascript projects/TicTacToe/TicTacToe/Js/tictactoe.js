...
function placeXOrO(squareNumber) { 
    ...
    // the .some() method is used to check each element of each selecSquare array 
    //to see if it contains the square clicked on.

    if (!selectedSquares.some(element=>element.includes(squareNumber))) {
        let select=document.getElementById(squareNumber);
        if (activePlayer==='X') {  //this condition checks who's turn it is. 
            select.style.backgroundImage='url("images/purplex.jpg")';  //If activePlayer is equal to X the x.png is placed in HTML.
             //Active player may only be 'X' or 'O' so if not 'X' it must be 'O'
        } else {
             //if activePlayer is equal to 'O' the o.png is placed in HTML 
            select.style.backgroundImage='url("images/greeno.jpg")';
        }
        ...
        audio('./media/bell.mp3'); //this function plays placement sound.
        if(activePlayer==='O') { //this condition check to see if it is the computer's turn.
            disableClick(); //this function disables clicking for computer's turn.
            setTimeout(function () { computersTurn();}, 1000); //this function wait 1 second before the computer places an image and enables click.
        }
        return true;  //returning true is needed for our computersTurn() function to work.
    }
    function computersTurn() { //this function result in a random square being selected by the computer.
        //this boolean is needed for our while loop/
        let success=false; //this variable stores a random number 0-8.
        let pickASquare; //this condition allows our while loop to keep trying if a square is selected already. 
        while(!success) {
            //All random numbers between 0 and 8 are selected. 
            pickASquare=String(Math.floor(Math.random() * 9));
            //If the random number evaluated returns true, the square hasn't been selected.
            if (placeXOrO(pickASquare)) {
                //this line calls the function.
                placeXOrO(pickASquare);
                //this line changes our boolean and ends the loop.
                success=true;
            };
        }
    }
...
function checkWinConditions() {
    ...
    //This function checks if an array includes 3 strings. It is used to check for each win.
    function arrayIncludes(squareA, squareB, squareC) {
        ...
        //true is returned and our else if condition execute the drawLine() function.
...
//This function utilizes HTML canvask to draw lines.
